...
sub set_parent {
    ...
    if ( ($self == $parent || UD::NodeC::is_descendant_of($parent, $self) )) {
...
sub remove {
    ...
    if ($arg_ref && $self->[$FIRSTCHILD]){
        ...
        if ($what_to_do =~ /^rehang/){
            foreach my $child (UD::NodeC::children($self)){
                UD::NodeC::set_parent($child, $parent);
            }
        }
        if ($what_to_do =~ /warn$/){
    ...
    my @to_remove = sort {$a->[$ORD] <=> $b->[$ORD]} ($self, UD::NodeC::_descendantsF($self));
...
sub create_child {
    my $self = shift;
    my $child = UD::NodeC->new(@_); #ref($self)->new(@_);
    UD::NodeC::set_parent($child, $self);
    return $child;
}
...
sub shift_before_node {
    ...
    return UD::NodeC::_shift_to_node($self, $reference_node, 0, 0, $arg_ref);
}

sub shift_after_node {
    my ( $self, $reference_node, $arg_ref ) = @_;
    return UD::NodeC::_shift_to_node($self, $reference_node, 1, 0, $arg_ref);
}

sub shift_before_subtree {
    my ( $self, $reference_node, $arg_ref ) = @_;
    return UD::NodeC::_shift_to_node($self, $reference_node, 0, 1, $arg_ref);
}

sub shift_after_subtree {
    my ( $self, $reference_node, $arg_ref ) = @_;
    return UD::NodeC::_shift_to_node($self, $reference_node, 1, 1, $arg_ref);
}
...
sub _shift_to_node {
    ...
    return if $skip_if_descendant && !$without_children && UD::NodeC::is_descendant_of($reference_node, $self);
    ...
    if ($subtree) {
        if ($without_children) {
            my $new_ref;
            if ($after) {
                foreach my $node ($reference_node, UD::NodeC::_descendantsF($reference_node)){
                    next if $node == $self;
            ...
            $reference_node = UD::NodeC::_descendants($reference_node, 1, !$after, $after, $self);
        }
    }
    ...
    my @nodes_to_move = UD::NodeC::_descendants($self, 1);
...
sub _shift_to_node {
    ...
    if ($without_children) {
        ...
        @nodes_to_move = UD::NodeC::_descendants($self, 1);
    }
    ...
    # Recompute ord of all nodes and reorder @$all_nodes (array of all nodes stored in root).
    # The technical root has ord=0, but it is not included in @$all_nodes.
    # So it should always hold that $all_nodes->[$n][$ORD] == $n.
    # It is faster (but more difficult to implement) to reorder @$all_nodes "in situ" and without using perl "sort".
    # We have one temporary array @nodes_to_move
    my $root = $self->[$ROOT];
    my $all_nodes = $root->[$DESCENDANTS];

    # For shift_before_*, we move the nodes before a node with ord=$reference_ord.
    # Let's convert shift_after_* to this case as well.
    my $reference_ord = $reference_node->[$ORD];
die "hh" if $reference_ord < 1;
    $reference_ord-- if $after;

    # Get ord of the first and last node of @nodes_to_move.
    # In projective trees $last_ord - $first_ord + 1 == @nodes_to_move,
    # but in non-projective there may be gaps in @nodes_to_move.
    my $first_ord = $nodes_to_move[0][$ORD];
    my $last_ord = $nodes_to_move[-1][$ORD];

    # Shift @nodes_to_move RIGHT (and other nodes in the range $first_ord..$reference_ord-1 LEFT).
    # So the node at position $reference_ord (and all the following nodes) stays there.
    # We'll have two while cycles which move one node at a time.
    if ($reference_ord > $first_ord) {
        my $trg_ord = $first_ord;
        my $src_ord = $first_ord+1;
        while ($is_moving{$all_nodes->[$src_ord-1]}){$src_ord++};

        # First, move a node from position $src_ord to position $trg_ord LEFT-ward.
        ...
        # Second, move @nodes_to_move to $trg_ord RIGHT-ward.
        #$src_ord = 0;
        #while ($trg_ord < $reference_ord) {
        #while ($src_ord <= $#nodes_to_move) {
        #   $all_nodes->[$trg_ord-1] = $nodes_to_move[$src_ord];
        #   $all_nodes->[$trg_ord-1][$ORD] = $trg_ord;
        #    $trg_ord++;
        #    $src_ord++;
        #}
    ...
    # Shift @nodes_to_move LEFT (and other nodes in the range $reference_ord .. $last_ord RIGHT).
    # So the node at position $first_ord-1 (and all the preceding nodes) stays there.
    else {
        my $trg_ord = $last_ord;
        ...
        while ($src_ord >= $reference_ord) {
            ...
            $trg_ord--;
            $src_ord--;
            while ($src_ord>1 && $is_moving{$all_nodes->[$src_ord-1]}){$src_ord--};
        }
        # Second, move @nodes_to_move to $trg_ord LEFT-ward.
#         $src_ord = $#nodes_to_move;
#         while ($src_ord > 0) {
#             $all_nodes->[$trg_ord-1] = $nodes_to_move[$src_ord];
#             $all_nodes->[$trg_ord-1][$ORD] = $trg_ord;
#             $trg_ord--;
#             $src_ord--;
#         }
        foreach my $node (reverse @nodes_to_move){
            $all_nodes->[$trg_ord-1] = $node;
            $node->[$ORD] = $trg_ord--;
        }
    }
