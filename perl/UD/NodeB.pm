...
sub set_parent {
    ...
    if ( ($self == $parent || UD::NodeB::is_descendant_of($parent, $self) )) {
...
sub remove {
    ...
    if ($arg_ref && $self->[$FIRSTCHILD]){
        ...
        if ($what_to_do =~ /^rehang/){
            foreach my $child (UD::NodeB::children($self)){
                UD::NodeB::set_parent($child, $parent);
            }
        }
        if ($what_to_do =~ /warn$/){
    ...
    my @to_remove = sort {$a->[$ORD] <=> $b->[$ORD]} ($self, UD::NodeB::_descendantsF($self));
...
sub create_child {
    my $self = shift;
    my $child = UD::NodeB->new(@_); #ref($self)->new(@_);
    UD::NodeB::set_parent($child, $self);
    return $child;
}
...
sub shift_before_node {
    ...
    return UD::NodeB::_shift_to_node($self, $reference_node, 0, 0, $arg_ref);
}

sub shift_after_node {
    my ( $self, $reference_node, $arg_ref ) = @_;
    return UD::NodeB::_shift_to_node($self, $reference_node, 1, 0, $arg_ref);
}

sub shift_before_subtree {
    my ( $self, $reference_node, $arg_ref ) = @_;
    return UD::NodeB::_shift_to_node($self, $reference_node, 0, 1, $arg_ref);
}

sub shift_after_subtree {
    my ( $self, $reference_node, $arg_ref ) = @_;
    return UD::NodeB::_shift_to_node($self, $reference_node, 1, 1, $arg_ref);
}
...
sub _shift_to_node {
    ...
    return if $skip_if_descendant && !$without_children && UD::NodeB::is_descendant_of($reference_node, $self);
    ...
    if ($subtree) {
        if ($without_children) {
            ...
            $reference_node = UD::NodeB::_descendants($reference_node, 1, !$after, $after, $self);
        }
    }
