...
sub set_parent {
    ...
    if ($origparent){
        if ($self == $origparent->[$FIRSTCHILD]) { # or if (!$self->[$PREVSIBLING])
...
sub remove {
    ...
    #$self->cut();
    if ($self == $parent->[$FIRSTCHILD]) { # or if (!$self->[$PREVSIBLING])
...
sub _shift_to_node {
    ...
    if (!$without_children && UD::NodeB::is_descendant_of($reference_node, $self)){
